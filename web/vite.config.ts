...
export default defineConfig({
  ...
  // base: "/" (vite default — kept). Earlier attempt to set base: "./"
  // for prefix-mounted deployment broke react-router on nested SPA URLs
  // like /mathran/projects/X/wiki/Y because <base href="/mathran/">
  // injected by portal interacts with relative URLs in ways that confuse
  // both the runtime asset loader AND react-router's history match. The
  // KaTeX font 404 issue this was meant to fix needs a different fix —
  // see portal-side patch to serve /assets/* as a public proxy passthrough
  // (commit pending). Reverted 2026-06-30 same session as introduction.
  base: "/",
  build: {
    outDir: "../dist/web",
    emptyOutDir: true,
