 ...
 * Each section spans `[marker.offset, nextMarker.offset)`.
 */
export function splitIntoSections(
  source: LoadedSource,
): Array<{ title: string; text: string; offset: number }> {
  const markers = source.sectionMarkers;
  const sections: Array<{ title: string; text: string; offset: number }> = [];
  for (let i = 0; i < markers.length; i++) {
    const start = markers[i].offset;
    const end = i + 1 < markers.length ? markers[i + 1].offset : source.text.length;
    sections.push({
...
export async function readPaperRegimeB(
  ...
  const log = deps.emitLog ?? (() => {});
  if (source.sectionMarkers.length < 2) {
  ...
  for (const sec of sections) {
    const sr = await readSection(paper, sec.title, sec.text, deps, readTitles, sec.offset);
