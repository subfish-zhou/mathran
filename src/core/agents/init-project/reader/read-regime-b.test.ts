...
function sectionedSource(): LoadedSource {
  return {
    ...
    truncated: false,
    sectionMarkers: [
      { title: "Introduction", level: 2, byteOffset: 0 },
      { title: "Main lemmas", level: 2, byteOffset: 4 },
      { title: "Proof", level: 2, byteOffset: 10 },
    ],
  };
}
...
describe("splitIntoSections", () => {
  it("splits text into per-marker chunks with correct boundaries and titles", () => {
    ...
    expect(sections.map((s) => s.byteOffset)).toEqual([0, 4, 10]);
  });
});
...
describe("readPaperRegimeB", () => {
  ...
  it("falls back to Regime A when there are fewer than 2 section markers", async () => {
    ...
    const source: LoadedSource = {
      ...
      bytes: 16,
      truncated: false,
      sectionMarkers: [{ title: "All", level: 2, byteOffset: 0 }],
    };
