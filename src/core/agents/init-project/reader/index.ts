...
import { readRegimeA } from "./read-regime-a.js";
import { readRegimeB } from "./read-regime-b.js";
import { readRegimeC } from "./read-regime-c.js";
...
async function runRead(
  ...
  if (regime === "B") {
    const body = await readRegimeB(args, deps);
    const sectionCount = args.skim.sectionOutline?.length ?? 0;
    return { body, llmCalls: Math.max(1, sectionCount) + 1 };
  }
  if (regime === "C") {
    return { body: await readRegimeC(args, deps), llmCalls: 1 };
  }
  return { body: await readRegimeA(args, deps), llmCalls: 1 };
}
...
export async function readPaper(paper: PaperNode, ctx: ReadPaperCtx): Promise<PaperRead> {
  ...
  try {
    skim = await skimPaper({ paper, source, problemTitle }, { llm, emitLog });
    totalLlmCalls += 1;
