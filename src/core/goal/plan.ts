...
export function formatPlanFragment(body: string): string {
  ...
  return [
    ...
    "Use it to decide what to work on next, and call `update_plan_item` to flip a",
    "checklist item between todo and done as you make progress. The index is the 1-based",
    "global position of the `- [ ]` bullet in the plan, in document order.",
    "",
    trimmed,
  ].join("\n");
}
